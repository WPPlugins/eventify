	...
	class eventify{
		...
		function options_panel() 
		{
			global $wpdb;
				...
				if(isset($_POST['emcateup']) && $_POST['emcateup']=="1")
				  {
			      
				    if(!get_category_by_slug($_POST['em_category_slug']))  $add_category_em = wp_insert_category(array('cat_name'=>ucwords(str_ireplace("_"," ",str_ireplace('-'," ",$_POST['em_category_slug']))),'category_description'=>'','category_nicename'=>str_ireplace(" ","-",strtolower($_POST['em_category_slug'])),'category_parent'=>''));
			    
				    update_option('em_category_slug',$_POST['em_category_slug']);
				    $message = "Category slug updated!";
				  }//emcateup if ends here
				 if(isset($_POST['eem']))
				{
				 ...
				 {
				 	$nonce = $_POST['nonce-eventify'];
					$repeats_event = (isset($_POST['em_repeats']) && !empty($_POST['em_repeats']))? $_POST['em_repeats'] : '0';
					if($repeats_event=="yes"){ $repeats_type=$_POST['em_repeats_type']; $repeats_until=$_POST['em_repeats_until'];}else {$repeats_type='0'; $repeats_until='0';}
					if (!wp_verify_nonce($nonce, 'eventify-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
				 	$message = "Event Added!";
					...
					// Create post object
			 			...
			 			$my_post['post_category'] = array(get_category_by_slug(get_option('em_category_slug'))->term_id);
			
						// Insert the post into the database
			 			$postedid = wp_insert_post( $my_post );
				
							$sql = "insert into ".$table_name." values(null,'".$_POST['datepicker']."','".$_POST['timepick_from']."-".$_POST['timepick_to']." ','".str_ireplace("'","\'",$_POST['em_desc'])."','".str_ireplace("'","\'",$_POST['em_title'])."','".str_ireplace("'","\'",$_POST['em_venue'])."','".$_POST['em_timezone']."','".$postedid."','".strtotime($_POST['datepicker'])."','".$repeats_event."','".$repeats_type."','".$repeats_until."')";
			 	
				 	}
				 	else
					{
					  
				 		$sql = "insert into ".$table_name." values(null,'".$_POST['datepicker']."','".$_POST['timepick_from']."-".$_POST['timepick_to']."','".str_ireplace("'","\'",$_POST['em_desc'])."','".str_ireplace("'","\'",$_POST['em_title'])."','".str_ireplace("'","\'",$_POST['em_venue'])."','".$_POST['em_timezone']."','0','".strtotime($_POST['datepicker'])."','".$repeats_event."','".$repeats_type."','".$repeats_until."')";
				 	}
				 	
					$wpdb->query($sql);
					$message .= $this->_repeat_event($wpdb->insert_id, $_POST);
				 	update_option("em_timezone",$_POST['em_timezone']);
					//$wpdb->show_errors(); //comment this line after done.
		 ...
		 if(isset($_POST['delem']) && $_POST['delem']=="1")
		 {
				if(!empty($_POST['delemid']))
				{
					...
					if(isset($_POST['deltype']) && $_POST['deltype']=="yes"){ $sql_qury = "select em_id from ".$table_name." where em_id in (".$delidsem.") or em_id in (select em_id from ".$table_name." where em_repeats in (select em_repeats from ".$table_name." where em_id in (".$delidsem."))) or em_id in (select em_repeats from ".$table_name." where em_id in (".$delidsem.")) or em_id in (select em_id from ".$table_name." where em_repeats in (".$delidsem."));"; 
					  $delidsem = $wpdb->get_col($sql_qury);
					  $delidsem = implode(",",$delidsem);
					  $sql_qury = "delete from ".$table_name." where em_id in (".$delidsem.")";
					}
		...
		/*code for backing up events*/
		 if(isset($_POST['backemall']) && $_POST['backemall']=="1")
		 {
			...
			$csv = new parseCSV();
				//print_r($results);
				$i=1;
				$data[0] = array('title' => 'title', 'date' => 'date', 'time' => 'time','desc'=>'desc','venue'=>'venue','timezone'=>'timezone','savetype'=>'savetype','timestamp'=>'timestamp','repeats'=>'repeats','repeats_type'=>'repeats_type','repeats_untill'=>'repeats_untill');
				foreach($results as $row)
				{
				  $data[$i] = array('title' => $row->em_title, 'date' => $row->em_date, 'timefromtimeto' => $row->em_time,'desc'=>$row->em_desc,'venue'=>$row->em_venue,'timezone'=>$row->em_timezone,'savetype'=>$row->em_savetype,'timestamp'=>$row->em_timestamp,'repeats'=>$row->em_repeats,'repeats_type'=>$row->em_repeats_type,'repeats_untill'=>$row->em_repeats_untill);
					$i++;	
				}
		...
		if(isset($_POST['uploadems']) && $_POST['uploadems']=="1") //code for bulk uploads
		{
		 	...
		 	if(isset($_POST['postedfpc']))
			{
				...
				if(($_FILES["csvfile"]["type"]=='text/plain' || $_FILES["csvfile"]["type"]=='text/csv') && (isAllowedExtension($_FILES['csvfile']['name'])))
				{	
					...
					if($no_of_cols!="6")
					{
						die('Uploaded file is in the wrong format.. <a href="http://eventify.zendesk.com" target="_blank">please click here for looking up tutorial videos over at the eventify support center..</a>');
					}
					global $wpdb;
					$table_name = $wpdb->prefix."em_main";
...
?>
		...
		<h4>If you use this plugin, I do humbly request you to donate via paypal using the below button.<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
		...
		"><input type="image" src="https://www.paypal.com/en_US/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!" stlye="margin:0;float:left;padding:0;position:relative;" align="left">
		<img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
		</form><br/><br/>It will help me to keep this plugin updated and managed. Along with coding and launching <a href="http://eventifypro.com" target="_blank">Eventify&trade; Pro</a>.<br> If you have any support requests or would like to know what others are requesting for eventify, <a href="http://eventify.zendesk.com" target="_new">click here</a>.</h4>
		
		<?php if ($message) : ?>
<div id="message" class="updated fade"><p><?php echo $message; ?></p></div>
<?php endif; ?><br/>

	<div style="float:left;clear:both;overflow:visible;clear:both;">
...
<script type="text/javascript">
	jQuery(function() {
	    var picker = new Pikaday(
                        {
                            ...
                            format: 'L',    // 06/29/2014
                            minDate: new Date(),
                            maxDate: function(){ mDate = new Date(); mDate.setFullYear(mDate.getFullYear()+3); return mDate; },
                            yearRange: [2014,2024]
                        });
                        
                        
        var picker_repeats = new Pikaday({
            
            field: document.getElementById("em_repeats_until"),
            firstDay: 1,
            format: 'L',    // 06/29/2014
            minDate: new Date(),
            maxDate: function(){ mDate = new Date(); mDate.setFullYear(mDate.getFullYear()+3); return mDate; },
		...
		jQuery("#category_slug_form").validate();
	});
	</script>
	<Style>
div.row { padding-top: 5px; clear: both; width:960px;} div.left { float: left;  text-align: left;margin-left:20px; width:180px;} div.right { margin-left: 110px; }
div.time-picker {
  position: absolute;
  height: 200px;
...
<form action="" method="post" id="backeventifyform">
	...
	<div class="row">  
		...
		<div class="right"><input type="text" size="25" class="datepicker" id="datepicker" name="datepicker" READONLY class="required" <?php if($isedit){?> value="<?php echo $editrow[0]->em_date; ?>" <?php } ?>>*For displaying the date in a particular format use the widget :)</div>
	</div>
	...
	<div class="row"> 
		...
		<?php if(!$isedit){ ?>
<div class="row"> 
		<div class="left"><strong>Repeat Event?:</strong></div>  
		<div class="right">Yes:<input type="radio" class="em_repeats" name="em_repeats" value="yes"> No:<input type="radio" class="em_repeats" name="em_repeats" value="0" CHECKED> </div> 
	</div> 
<div class="row repeat_event"> 
		<div class="left "><strong>Repeats When:</strong></div>  
		<div class="right"><select name="em_repeats_type" id="em_repeats_type"><option value="1">Daily</option><option value="7">Weekly</option><option value="15">Bi-Weekly</option><option value="30">Monthly</option><option value="60">Bi-Monthly</option><option value="120">Quaterly</option><option value="182">Half Yearly</option><option value="365">Yearly</option></select> </div> 
		
	</div> 
	<div class="row repeat_event">
	     <div class="left"><strong>Repeats untill:</strong></div>
	     <div class="right"><input type="text" name="em_repeats_until" id="em_repeats_until"  READONLY  class="datepicker" id="repeatpicker"/> </div>
	</div>	    <?php } ?>
	<div class="row"> 
...
?>
<br/><br/>
<h3>Category Slug For Events Saved As Blog Posts</h3>
	   <p>Please enter a category slug, if the category slug is not found, a category will be created.</p>
<form action="" method="post" id="category_slug_form" name="category_slug_form">
<input type="text" name="em_category_slug" id="em_category_slug" class="required" value="<?php echo get_option('em_category_slug');?>" />
<input type="hidden" name="emcateup" id="emcateup" value="1" />
<input type="hidden" name="nonce-eventify" value="<?php echo wp_create_nonce('eventify-nonce'); ?>" />
<input type="submit" value="Update Category Slug" name="emcategoryslug"/>
</form>

<br/><br/>
<h3>Bulk Upload Events (See <a href="http://zendesk.eventify.com" target="blank">Eventify&trade; Legacy Support</a> )</h3>
	   <p><em>Note:</em> The bulk upload event features works, but is limited, you can only upload events that are "pop-ups" and do not repeat. <a href="<?php echo plugins_url('eventify/uploads/EventifyEg.csv'); ?>">Click here (right click and "save as..") </a> to download an example csv file that works correctly with this feature.</p>
...
<h3>Back up Events? See HowTo video to Backup</h3>
																	  <p>In case you want to back up your events, repeating events, and all the other information about your events, you can simply click the button, and
																	  download the csv file from the link shown in the message area on the top of the page (visibile only after you have clicked the button). To restore all the events back, you can use MySQL directly or use PHPMyAdmin to do so.</p>
...
<form action="POST">
<div class="row">
<div class="left"><div id="deleting"></div>Delete Associated Repeating Events?<input type="checkbox" value="yes" id="deltype" name="deltype"/></div>  
...
function postbackdel()
{
	...
	var deltyp = jQuery("#deltype:checked").val();
	var allVals = [];
	...
	jQuery.post(url,{nonceeventify: nonce,deltype: deltyp, delem: delm, 'delemid[]': allVals  },function(data){jQuery("#deleting").html("<em>Event Deleted Reloading...<\/em>");location.reload(true);},"text");
...
<input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
">
Donate and Buy me a beer.<input type="image" src="https://www.paypal.com/en_US/i/btn/btn_donate_SM.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!" stlye="margin:0;float:left;padding:0;position:relative;" align="left">
<img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
</form> </div><div style="float:left"><br><br>Need help? Use twitter to raise ticket(s)  <a target="_blank" title="Follow me on twitter" href="http://twitter.com/eventifypro">Eventify&trade; Pro on twitter.</a>. Alternative you can <a href="http://facebook.com/eventifypro" target="_blank">post at the Eventify&trade; facebook page</a> or <a href="mailto:support@eventify.zendesk.com" target="_blank">email support</a>.<br/><br/><h4>&copy; Plugin by <a href="http://sojourner.co" target="_blank">Sojourner.co</a></h4></div>
...
<?php	} //end of options_panel()
	...
	{	global $subpage;
		if(is_admin())
		{
				...
				wp_register_script('adminjs',$url.'adminmain.js',false,''); #check whats in here.
				wp_enqueue_script('adminjs'); #check whats in here.
		...
		function eventifyform_func($atts)
		{
			...
			$eventform='<script type="text/javascript">
				jQuery(function() {
                        ...
                        var picker_repeats1 = new Pikaday({
			...
			#em_timezone
			{
				...
				<div class="row">  
					<div class="left"><strong>Event Date:</strong></div> 
					<div class="right"><input type="text" size="27" id="datepicker" name="datepicker" READONLY class="required datepicker"></div>
				</div>


				<div class="row"> 
					<div class="left"><strong>Time - From</strong></div>  
					<div class="right"><input type="text" size="27" id="timepick_from" name="timepick_from" READONLY class="required"></div>
				</div> 
				<div class="row">
					<div class="left"><strong>Time - To</strong></div>
					<div class="right"><input type="text" size="27" id="timepick_to" name="timepick_to" READONLY class="required"></div>
				</div>
				<div class="row">
					<div class="left"><strong>Event Venue:</strong></div>  
...
<div class="row repeat_event"> 
	...
	<div class="row repeat_event">
			...
			foreach($results as $row){
				...
				{  
					...
					else
					{
						...
						$eventlisted.="<li> ".$rowemdate." - <a href=\"".$row->em_id."\" rel=\"#dialog\" name=\"modal\">".$row->em_title."</a> </li>";
					}
			...
			while($countloop>0)
			{
				if(strtotime($results[$arr_i]->em_date)>=$date_today)
				{
					...
					else
					{
						...
						$eventlisted.="<li> ".$rowemdate." - <a href=\"".$results[$arr_i]->em_id."\" rel=\"#dialog\" name=\"modal\">".$results[$arr_i]->em_title."</a> </li>";
					}
		...
		function eventifycal_func($atts)
		{
			//getting the atts from the shortcode
			$caldrw=""; //var to return the draw fullcal on the page where the shortcode is used :D 
			$atts_ret = extract(shortcode_atts(array(
  ...
  return n.length >= width ? n : new Array(width - n.length + 1).join(z) + n;
				...
				//remove these 4 lines	//var date = new Date();
					//var d = date.getDate();
					//var m = date.getMonth();
					//var y = date.getFullYear();
					
					jQuery(\'#calendarem\').fullCalendar({
						header: {
													left:   \'title\',
			...
			foreach($results as $event):
					$arr_date = explode("/",$event->em_date);
					$arr_time = explode("-",$event->em_time);
					$arr_time_start = explode(":",$arr_time[0]);
					$arr_time_end = explode(":",$arr_time[1]);
				
					$caldrw .='{
						title: \''.nl2br($event->em_title).'\',
						start: new Date('.$arr_date[2].','.($arr_date[0]-1).','.$arr_date[1].','.$arr_time_start[0].','.$arr_time_start[1].'),
						end: new Date('.$arr_date[2].','.($arr_date[0]-1).','.$arr_date[1].','.$arr_time_end[0].','.$arr_time_end[1].'),
						allDay:false,
						description: \''.str_replace(array("\r\n", "\r", "\n", "\t"), ' ',$event->em_desc).'\',
						venue: \''.str_replace(array("\r\n", "\r", "\n", "\t"), ' ',$event->em_venue).'\',
						timezone: \''.$event->em_timezone.'\'
				...
				endforeach;		
					...
					$caldrw.='		
						...
						eventRender: function(event,element)
						{
							...
							element.qtip({
								content : \'<h4 class="bubbletext">\'+event.title+\'</h4><span class="starttd"><strong>Start: </strong>\'+new Date(event.start).toDateString()+\' <br>\'+new Date(event.start).getHours() +\':\'+ Npad(new Date(event.start).getMinutes(),2)+\'</span><br/><span class="endid"><strong>End: </strong>\'+new Date(event.end).toDateString()+\' <br>\'+new Date(event.end).getHours() +\':\'+ Npad(new Date(event.end).getMinutes(),2)+\'</span><br/><span class="eventdescription"><strong>Description:</strong>\'+event.description+\'</span><br/><span class="eventvenue"><strong>Venue:</strong>\'+event.venue+\'</span><br/><span class="eventtimezone"><strong>Timezone:</strong>\'+event.timezone+\'</span><br/>\',
							...
							length = awd.push(element.children(\'a\').css(\'width\'));
							length = aht.push(element.children(\'a\').css(\'height\'));
						...
						{
							evei++;
							element.children(\'a\').css(\'width\',awd[evei]);
							element.children(\'a\').css(\'height\',aht[evei]);
...
//function to check and add repeated events in the database simply
               function _repeat_event($parent_id,$post){
		 if($post['em_repeats']=="yes"){
		    $str_event_time = strtotime($post['datepicker']);
		   
		   $str_event_until_time = strtotime($post['em_repeats_until']);
		   global $wpdb;
		   ...
		   $month_advance =0;
		   $day_advance = 0;
		   if($post['em_repeats_type']=="1" || $post['em_repeats_type']<=15) $day_advance = $post['em_repeats_type'];
		   if($post['em_repeats_type']>15 && $post['em_repeats_type']<=183) $month_advance = $post['em_repeats_type']/30;
		   if($post['em_repeats_type']>183 && $post['em_repeats_type']<=365) $year_advance = $post['em_repeats_type']/365;*/
		  
		   
		   //echo date("m/d/Y",mktime('0','0','0',date("m",$str_event_time),date("d",$str_event_time)+$post['em_repeats_type'],date("Y",$str_event_time))); 
		   
		   if($str_event_time >= $str_event_until_time) $str_event_until_time = mktime('0','0','0',date("m",$str_event_time),date("d",$str_event_time)+$post['em_repeats_type'],date("Y",$str_event_time));
		   
		   
		   $loopTime =  mktime('0','0','0',date("m",$str_event_time),date("d",$str_event_time)+$post['em_repeats_type'],date("Y",$str_event_time));
		   while($loopTime<=$str_event_until_time){
		       if(isset($post['addem']) && $post['addem']=="1")
				 {
				 	$nonce = $post['nonce-eventify'];
					if (!wp_verify_nonce($nonce, 'eventify-nonce')) die ( 'Security Check - If you receive this in error, log out and back in to WordPress');
				 	
		 	
				 	//add the event to the db and the table :)
				 	$table_name = $wpdb->prefix."em_main";
					if($post['saveitaspopup']=="posted")
					{
					// Create post object
			
			  			$my_post = array();
			  			$my_post['post_title'] = $post['em_title'];
			  			$my_post['post_content'] = "<h2>Event Description:</h2>".$post['em_desc']."<br/><h2>Event Date</h2>".date('m/d/Y',$loopTime)."<br/><h2>Event Time</h2> From: ".$post['timepick_from']." - To: ".$post['timepick_to']." <br/><h2>Event Venue</h2>".$post['em_venue']."<br/><h2>Event TimeZone</h2>".$post['em_timezone'];
			  			$my_post['post_status'] = 'publish';
			  			$my_post['post_author'] = 1;
			 			$my_post['post_category'] = array(get_category_by_slug(get_option('em_category_slug'))->term_id);
						...
						$sql = "insert into ".$table_name." values(null,'".date('m/d/Y',$loopTime)."','".$post['timepick_from']."-".$post['timepick_to']." ','".str_ireplace("'","\'",$post['em_desc'])."','".str_ireplace("'","\'",$post['em_title'])."','".str_ireplace("'","\'",$post['em_venue'])."','".$post['em_timezone']."','".$postedid."','".$loopTime."','".$parent_id."','0','0')";
			 	
				 	}
				 	else
					{
					  
					  $sql = "insert into ".$table_name." values(null,'".date('m/d/Y',$loopTime)."','".$post['timepick_from']."-".$post['timepick_to']."','".str_ireplace("'","\'",$post['em_desc'])."','".str_ireplace("'","\'",$post['em_title'])."','".str_ireplace("'","\'",$post['em_venue'])."','".$post['em_timezone']."','0','".$loopTime."','".$parent_id."','0','0')";
				 	}
		     ...
		     $loopTime = mktime('0','0','0',date("m",$loopTime),date("d",$loopTime)+$post['em_repeats_type'],date("y",$loopTime));
		   }
		   //print_r($post);
